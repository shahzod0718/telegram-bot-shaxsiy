...
async def send_video(message: types.Message):
    ...
    if await check_subs(user_id):  # Faqat obuna bo‘lganlarga javob qaytaradi
        file_id = "BAACAgQAAxkBAAIIXGfRj9m6AAFulrC_7ELuP46t1BwBrgACShkAAsjZYFOlNNHuusIlqDYE"
        await message.answer_video(file_id, caption="🎬 Bedlend ovchilar")
    else:
        await message.answer('telegram kanalga obuna boling')
        markup = InlineKeyboardMarkup(
...
async def send_video(message: types.Message):
    ...
    if await check_subs(user_id):  # Faqat obuna bo‘lganlarga javob qaytaradi
        file_id = "BAACAgQAAxkBAAIIf2fR-UAcd6FmYnewIdMjuEtL6b_pAAIUDAACqUQRUG9LWpGZOSEsNgQ"
        await message.answer_video(file_id, caption="🎬 O'zga Olam")
    else:
        await message.answer('telegram kanalga obuna boling')
        markup = InlineKeyboardMarkup(
            inline_keyboard=[
...
async def send_video(message: types.Message):
    ...
    if await check_subs(user_id):  # Faqat obuna bo‘lganlarga javob qaytaradi
        file_id = "BAACAgIAAxkBAAIIi2fSTAQdx9crU9FoOnJua23FVRW8AAL5YgAC41WQStfsOhFMM2TnNgQ"
        await message.answer_video(file_id, caption="🎬Jinoyatchi– jinoyat olamiga kirib qolgan shaxsning hayoti 🔥")
...
async def send_video(message: types.Message):
    ...
    if await check_subs(user_id):  # Faqat obuna bo‘lganlarga javob qaytaradi
        file_id = "BAACAgIAAxkBAAIIe2fR9quxvXcdq8zUmXZmlkFxVfAyAAITYgAC41WQSioFumr_pezGNgQ"
        await message.answer_video(file_id, caption="🎬 Telba Qasoskor – Adolat yo‘lida kechgan jang! 🔥")
...
async def send_video(message: types.Message):
    ...
    if await check_subs(user_id):  # Faqat obuna bo‘lganlarga javob qaytaradi
        file_id = "BAACAgIAAxkBAAIIqWfSxKBOuHIKXc3feCbqmoCjH2dDAAIdDwACUubxSMgniBx2LfaINgQ"
        await message.answer_video(file_id, caption="🎬 Film: Biz hayvonot bog'ini sotib oldik 🔥")
